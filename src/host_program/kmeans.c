 ...
 */
void read_bin_input(Params *p,             /**< Algorithm parameters */
 ...
 */
void read_txt_input(Params *p,             /**< Algorithm parameters */
                    const char *filename,  /**< [in] The file name. */
                    float ***features_out) /**< [out] Vector of features. */
{
  char line[1024];
...
#pragma omp parallel for collapse(2) reduction(+ : mean[:p->nfeatures])
...
#pragma omp parallel for collapse(2) reduction(max : max_feature)
  for (ipoint = 0; ipoint < p->npoints; ipoint++)
    for (ifeature = 0; ifeature < p->nfeatures; ifeature++)
  ...
  features_int[0] =
      (int_feature *)malloc(p->npadded * p->nfeatures * sizeof(features_int));
...
#pragma omp parallel for collapse(2)
  for (ipoint = 0; ipoint < p->npoints; ipoint++)
    for (ifeature = 0; ifeature < p->nfeatures; ifeature++)
      features_int[ipoint][ifeature] =
          lroundf(features_float[ipoint][ifeature] * p->scale_factor);
...
#pragma omp parallel for collapse(2) reduction(+ : variance[:p->nfeatures])
  for (ipoint = 0; ipoint < p->npoints; ipoint++)
    for (ifeature = 0; ifeature < p->nfeatures; ifeature++)
      variance[ifeature] +=
          features_float[ipoint][ifeature] * features_float[ipoint][ifeature];
...
static void error_check(Params *p) {
  ...
  if ((p->max_nclusters < p->min_nclusters) ||
      (p->max_nclusters > ASSUMED_NR_CLUSTERS)) {
    printf(
        "Error: min_nclusters(%d) > max_nclusters(%lu) or max_nclusters > max "
        "clusters allowed(%d) -- cannot proceed\n",
        p->min_nclusters, p->npoints, ASSUMED_NR_CLUSTERS);
    exit(EXIT_FAILURE);
  }
  if (ASSUMED_NR_FEATURES < p->nfeatures) {
    printf(
        "Error: nfeatures(%d) > max clusters allowed(%d) -- cannot proceed\n",
        p->nfeatures, ASSUMED_NR_FEATURES);
    exit(EXIT_FAILURE);
  }
  if (p->npadded * p->nfeatures / p->ndpu > MAX_FEATURE_DPU) {
    printf(
        "Error: npadded*nfeatures/ndpu(%lu) > max features allowed per dpu(%d) "
        "-- cannot proceed\n",
 ...
 * @param cluster_centres [in] Coordinate of clusters centres for the best
 * iteration.
 * @return float* The return array
 */
static float *array_output(Params *p, int best_nclusters,
                           float **cluster_centres) {
#pragma omp parallel for collapse(2)
  for (int icluster = 0; icluster < best_nclusters; icluster++)
    for (int ifeature = 0; ifeature < p->nfeatures; ifeature++)
      cluster_centres[icluster][ifeature] =
          cluster_centres[icluster][ifeature] + p->mean[ifeature];
...
static void cli_output(
    Params *p,               /**< Algorithm parameters */
    float **cluster_centres, /**< [in] coordinate of clusters centres for the
                                best iteration */
    ...
    int index)  /**< [in] number of trials for the best RMSE */
{
  ...
  if (p->min_nclusters == p->max_nclusters && p->isRMSE) {
    if (p->nloops != 1) {  // single k, multiple iteration
    ...
    int *best_nclusters) /**< [out] best number of clusters according to RMSE */
{
  ...
  index = cluster(p,                /* Algorithm parameters */
  ...
  if (p->isOutput) cli_output(p, cluster_centres, rmse, index);
