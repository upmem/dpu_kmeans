...
static int64_t
    new_centers_len[ASSUMED_NR_CLUSTERS]; /**< [nclusters]: no. of points in
                                             each cluster */
static int64_t
    new_centers[ASSUMED_NR_CLUSTERS]
               [ASSUMED_NR_FEATURES]; /**< coordinates of the centroids */
...
void allocateClusters(Params *p, unsigned int nclusters) {
  ...
  clusters_int =
      (int_feature **)malloc(nclusters_round * sizeof(*clusters_int));
  clusters_int[0] = (int_feature *)malloc(nclusters_round * p->nfeatures *
                                          sizeof(**clusters_int));
   ...
   */
  old_clusters_float =
      (float **)malloc(nclusters * sizeof(*old_clusters_float));
  old_clusters_float[0] =
      (float *)malloc(nclusters * p->nfeatures * sizeof(**old_clusters_float));
  for (int i = 1; i < nclusters; i++)
 ...
 *
 */
void deallocateMembershipTable() { free(membership); }

/**
 * @brief Performs a final reduction of the centroids coordinates in float
 * format.
 */
    ...
    dpu_set *allset)        /**< pointer to the set of all assigned DPUs */
{
  ...
  DPU_ASSERT(dpu_push_xfer(*allset, DPU_XFER_FROM_DPU, "t_membership", 0,
                           npointperdpu * sizeof(*membership),
                           DPU_XFER_DEFAULT));
  ...
  memset(clusters_float[0], 0,
         sizeof(**clusters_float) * nclusters * nfeatures);

#pragma omp parallel
  {
    float **clusters_thread =
        (float **)malloc(nclusters * sizeof(*clusters_thread));
    clusters_thread[0] =
        (float *)calloc(nclusters * nfeatures, sizeof(**clusters_thread));
    for (int i = 1; i < nclusters; i++)
...
#pragma omp for schedule(static) collapse(2)
    for (int ipoint = 0; ipoint < npoints; ipoint++)
      for (int ifeature = 0; ifeature < nfeatures; ifeature++)
        clusters_thread[membership[ipoint]][ifeature] +=
            features_float[ipoint][ifeature];

#pragma omp critical
    for (int icluster = 0; icluster < nclusters; icluster++)
      for (int ifeature = 0; ifeature < nfeatures; ifeature++)
        clusters_float[icluster][ifeature] +=
            clusters_thread[icluster][ifeature];
    ...
    int i_init)                 /**< index of current outer iteration */
{
  ...
  if (nclusters > p->npoints) nclusters = p->npoints;
  ...
  clusters_float[0] =
      (float *)malloc(nclusters * p->nfeatures * sizeof(**clusters_float));
  for (int i = 1; i < nclusters; i++)
  ...
  do {
    DPU_ASSERT(
        dpu_broadcast_to(p->allset, "c_clusters", 0, clusters_int[0],
                         nclusters_round * p->nfeatures * sizeof(int_feature),
                         DPU_XFER_DEFAULT));
    ...
    kmeansDpu(
        p, nclusters,    /* number of clusters k */
    ...
    // #pragma omp parallel for collapse(2) reduction(+:frob) // awful
    // performance, don't do that
    for (int i = 0; i < nclusters; i++)
      for (int j = 0; j < p->nfeatures; j++)
        if (new_centers_len[i] > 0) {
          float new_coordinate =
              (float)new_centers[i][j] /
    ...
    (*loop)++;
  } while (((*loop < p->max_iter) && (frob > p->threshold)) ||
           *loop == 1); /* makes sure loop terminates */

  if (p->isOutput) printf("iterated %d times\n", *loop);

#ifdef FLT_REDUCE
  final_reduction(features_float, nfeatures, npoints, npadded, nclusters, ndpu,
                  clusters_float, allset);
#endif
